...
def _strip_leading_subheadline(body: Optional[str], description: Optional[str]) -> Optional[str]:
    ...
    of the article body. Publishers commonly reuse that same line as the
    page's meta description, so if the first line of the body matches the
    article's meta description, it's the dek — drop it."""
    ...
    if not desc_norm:
        return body
    lines = body.split("\n")
    if lines and _normalize_for_match(lines[0]) == desc_norm:
        return "\n".join(lines[1:])
    return body
...
def _strip_leading_duplicate_lines(
    ...
    headline/author/description, or match a generic desk/wire byline
...
def scrape(url: str, delay: float = 0.0) -> ArticleResult:
    ...
    body = _strip_leading_duplicate_lines(body, headline, author, description)
    body = _strip_duplicate_headline_lines(body, headline)
    body = _strip_leading_subheadline(body, description)
