...
from print_scraper import parse_table_paste, fetch_table_row, SOURCE_LABELS, SUPPORTED_SOURCES, EXCLUDED_SOURCES
...
<script>
  ...
  function copyAll() {
    ...
    // Skip null entries (error rows) but keep everything else in order.
    const rows = articles.filter(a => a).map(a => tsvEscape(formattedText(a)));
...
def index():
    ...
    if request.method == "POST":
        ...
        for i, url in enumerate(urls):
            result = scrape(url, delay=1.0 if i > 0 else 0)
            results.append(result)

    articles_json = json.dumps(
        [None if r.error else asdict(r) for r in results],
        ensure_ascii=False,
    ).replace("</", "<\\/")
...
def print_index():
    ...
    if request.method == "POST":
        ...
        for i, row in enumerate(parsed_rows):
            result = fetch_table_row(row, delay=1.0 if i > 0 else 0, json_cache=json_cache)
            results.append(result)

    bodies_json = json.dumps(
        [r.body if r.status == "ok" else None for r in results],
        ensure_ascii=False,
    ).replace("</", "<\\/")
